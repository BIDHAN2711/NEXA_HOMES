...
$data = ['id' => '', 'Name' => '', 'Phone' => '', 'Location' => '', 'Email' => ''];

if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $stmt = $conn->prepare("SELECT * FROM realstate_enquiry WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $name = $_POST['name'];
    $city = $_POST['phone'];
    $mobile = $_POST['location'];
    $email = $_POST['email'];

    $stmt = $conn->prepare("UPDATE realstate_enquiry SET Name = ?, Phone = ?, Location = ?, Email = ? WHERE id = ?");
...
<form method="POST" class="max-w-xl mx-auto bg-white p-6 rounded-2xl shadow space-y-5">
    ...
    <div>
        ...
        <input type="text" id="city" name="phone" value="<?= htmlspecialchars($data['Phone']) ?>"
               class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>

    <div>
        <label for="mobile" class="block text-sm font-medium text-gray-700 mb-1">Mobile</label>
        <input type="text" id="mobile" name="location" value="<?= htmlspecialchars($data['Location']) ?>"
               class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
